...
Vue를 이용한 개발을 좀 더 쉽게 만들어주는 유틸리기 기능인 Mapper Helper를 배워보겠습니다.
...
`mapGetters` 함수는 배열을 파라미터로 받고, Getter의 이름을 문자열로 받으며 객체를 반환합니다. 

그리고, `...`를 사용해 객체를 Computed 내부로 퍼뜨릴 수 있습니다.

<br>

동일하게 일반 함수에서 Vuex의 Actions를 사용하는 `dispatch()` 함수도 `mapActions` 함수를 이용해 동일한 방법으로 사용할 수 있습니다.

이 `mapActions` 함수도 파라미터로 배열을 받고 Actions의 이름을 문자열로 받으며 객체를 반환하는 것도 똑같습니다.

하지만 `mapActions`의 Action 이름을 함수명으로 쓰기 싫다면 `mapActions` 함수의 파라미터에 객체를 넣어줍니다.

```js
<script setup>
import { mapActions } from 'vuex';

const { inc } = mapActions(['increment']);
</script>
```