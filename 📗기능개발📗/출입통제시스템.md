...
서버가 죽으면 60초마다 프로세스를 확인해 **java.exe**가 실행되어 있지 않으면 자동으로 재시작 해주는 배치파일을 작성합니다.
...
@echo off
:loop
tasklist /FI "IMAGENAME eq java.exe" | find /i "count.jar" > nul
if errorlevel 1 (
  start "" java -jar "E:\Project\count.jar"
)
