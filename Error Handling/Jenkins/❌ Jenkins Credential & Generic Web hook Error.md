...
일단 에러의 내용이 ECDSA 키가 없다고 했으니 등록해봅시다.
...
위의 명령어가 기본적인 해결책이지만, 저는 다른 문제도 있었습니다. (권한, 파일 소유자, SSH 키 변경 기록 등)

<br>

저는 EC2 내부에 Jenkins를 컨테이너로 올려서 사용하다가, 안올려서 사용해봤는데

이때 jenkins가 설치되는 디렉터리는 /var/lib/jenkins 였습니다.

근데 jenkins 폴더에서 ssh key를 만들때 EC2의 Root계정으로 만들었습니다.

그럼 당연히jenkins 유저가 root 소유의 키 파일을 사용할 수 없을겁니다.
...
<br>
이래도 안된다면 SSH Key 변경을 했던게 떠올라서 SSH Key를 업데이트 해줍니다.
...
Jenkins의 Plugin 중 Generic Webhook Trigger를 사용 중,
