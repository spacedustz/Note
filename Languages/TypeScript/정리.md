...
tuple = ['a', 1];tuple = ['a', 1, 2]; // error - TS2322tuple = [1, 'a']; // error - TS2322```  
...
```ts  
let tuple: [1, number];  
tuple = [1,2];tuple = [1,3];tuple = [2,3]; // error - TS2322: Type '2' is not assignable to type '1'```  
...
```ts  
let tuple: [string, number];  
tuple = ['a', 1];tuple = ['b', 2];tuple.push(3);  
...
export default function getItems(user: User): Result {    // Some Logic..    if (id.isValid) {        return {            success: true,  
            value: ['Apple', 'Banana']  
...
const skw: User & Validation = {    name: 'skw',  
    age: 1,  
...
```ts  
// 초기화된 변수  
let num = 12;num = 'Hello Type'; // error-TS2322 : Type 'Hello Type' is not assignable to type 'number'  
// 기본값이 설정된 매개 변수 'b'function add(a: number, b: number = 2): number {  
    // 반환값이 있는 함수  
    return a + b;  
}  
```  
...
// Type Assertion
(document.querySelector('.menu-item')) as HTMLDi
```
