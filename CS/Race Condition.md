...
### **not_turn 변수 추가로 인한 해결책**

변수 이름처럼 ''아직 내 차례가 아님'' 을 나타내는 변수이다.

not_turn에 대응되는 프로세스는 임계구역에 진입하지 못하며, not_turn도 전역변수이다.

코드의 흐름을 보면 i가 자원을 사용하겠다고 선언하고 바로 다음에 i 를 Lock 걸어버린다

not_turn은 전역변수이기 때문에 무조건 하나의 프로세스만 Lock 된다 (상호 배제 해결)

진입 직전에 not_turn을 갱신하기 때문에 임계구역에 진입하지 못하는 상황이 없다 (한계 대기 해결)
...
진입 직전에 not_turn 변수를 갱신하기 때문에 임계구역에 진입하지 못하는 상황은 발생하지 않는다

not_turn 변수 파트와 내용 동일
