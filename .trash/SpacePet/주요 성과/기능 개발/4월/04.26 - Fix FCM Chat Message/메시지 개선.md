...
앱 → 서버 → FCM 서버 → 앱 (알림) 의 순으로 이루어져 있다

이를 앱 → FCM 서버가 가능하게 하려면 (HTTPS API가 존재한다) 이루어져야 하는 

Flow Chart와 수도코드를 간단히 작성해본다.

<br>

주의사항

- 유저의 토큰이 만료 (메시지 전송이 실패하였을 경우) 되었을 경우 해야하는 방안 강구
  (서버에서 처리해야한다 - 서버의 토큰은 항상 최신화이며 서버 토큰도 만료되었을 경우
   유저가 앱을 삭제하거나 로그아웃, 탈퇴한 경우이다)
- 유저의 인터넷 상황으로 인해 실패했을 경우 설계 방안 강구
- 앱에서 처리해야 하는 코드는 작성하지 않아도 된다
...
- key -> Bearer Access Token (GoogleCredentials  클래스), Header 추가 (HttpURLConnection 클래스)

### 전송 요청 페이로드 업데이트

- (`to` + path ) -> message, topic, news
