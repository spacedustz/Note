...
public class InitSchemaLoader implements ApplicationRunner {  
    ...
    public void run(ApplicationArguments args) throws Exception {  
        ...
        if (eventRepository.count() == 0) {  
            Event event = Event.createOf(LocalDateTime.now());  
            event.setInCount(0);  
            event.setOutCount(0);  
  
            try {  
                eventRepository.save(event);  
                log.info("Event Repository 내 컬럼 수: {} - 객체 생성 완료, Event ID: {}", eventRepository.count(), event.getId());  
            } catch (Exception e) {  
                log.error("Event Repository 내 컬럼 수: {} - 객체 생성 실패", eventRepository.count(), e);  
        ...
        if (eventRepository.count() > 0) {  
            ...
            try {  
                ...
                assert storedEvent != null;  
                log.info("현재 날짜에 이미 생성된 객체가 있는지 확인 중 ... Event ID: {}", storedEvent.getId());  
            } catch (Exception e) {  
  
            }  
            LocalDate currentDate = LocalDate.now();  
  
            if (!storedEvent.getEventTime().toString().contains(currentDate.toString())) {  
                Event event = Event.createOf(LocalDateTime.now());  
                event.setInCount(0);  
                event.setOutCount(0);  
                ...
                try {  
                    eventRepository.save(event);  
                    log.info("Event Repository 내 컬럼 수: {} 이상, 현재 시간 불일치 - 객체 생성 완료, Event ID: {}", eventRepository.count(), event.getId());  
                } catch (Exception e) {  
                    log.error("Event Repository 내 컬럼 수: {} 이상, 현재 시간 불일치 - 객체 생성 실패", eventRepository.count(), e);  
                    throw new CommonException("INIT-001", HttpStatus.BAD_REQUEST);  
                }  
            }  
        }  
