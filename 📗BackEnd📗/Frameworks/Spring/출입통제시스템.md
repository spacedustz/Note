...
function loadInitialData() {  
    fetchJson(httpUrl + '/init')  
        .then(initialRoomInfo => {  
            ...
            displayOccupancy(roomInfo.occupancy, roomInfo.maxCount);  
        });  
}  
...
function updateRoomInfo(updatedData) {  
    ...
    displayOccupancy(updatedData.occupancy, updatedData.maxCount)  
}  
...
// 방안의 현재 인원  
function displayOccupancy(occupancy, maxCount){  
    document.getElementById('count').innerText= occupancy + " / " + maxCount ;  
}  
...
화면을 보면 Spring의 소켓에 접속해 Event 객체를 받아 객체의 값을 잘 가져옵니다.
