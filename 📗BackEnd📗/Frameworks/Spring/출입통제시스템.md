    ...
    <script type="text/javascript">  
        ...
        stompClient.connect({}, function (frame) {  
            ...
            stompClient.subscribe('/count/data', function (data) {  
                eventData = JSON.parse(data.body); // 실시간 데이터 파싱  
                updateCount(eventData);  
            });  
        });  
  
        // 브라우저 첫 load 시, 초기 Event 객체 값 Load        function init() {  
            window.fetch('http://localhost:8090/ws/init', {  
                method: 'GET',  
                headers: {  
                ...
                .then(data => {  
                    initEvent.id = data.id;  
                    initEvent.occupancy = data.occupancy;  
                    initEvent.maxCount = data.maxCount;  
                    initEvent.status = data.status;  
  
                    console.log("초기 Event 데이터 로드 완료");  
  
                    document.getElementById('status').innerText = initEvent.status;  
                    document.getElementById('count').innerText = initEvent.occupancy + " / " + initEvent.maxCount;  
                });  
        }  
  
        // Life Cycle Hook, 브라우저 첫 로드 시 init 함수 호출  
        ...
        // 데이터의 Status 값을 변경하는 함수  
        function updateStatus(eventData) {  
            var newStatus = document.getElementById('new-status').value;  
  
            window.fetch('http://localhost:8090/ws/update-status?status=' + newStatus, {  
                method: 'PATCH',  
                headers: {  
                ...
                .then(response => eventStatus = response)  
                .then(data => console.log('Status 변경 완료 : ', data.status));  
            init();  
