...
public class InitSchemaLoader implements ApplicationRunner {  
    ...
    public void run(ApplicationArguments args) throws Exception {  
        ...
        if (objectCount > 0) {  
            log.info("Event Table 내부에 데이터가 있습니다. 객체의 날짜가 현재 날짜와 동일한 지 검증 중입니다...");  
            ...
            try {  
                ...
                log.error("현재 날짜에 해당하는 데이터가 이미 존재합니다.");  
            ...
            if (!storedEvent.getEventTime().toString().contains(currentDate.toString())) {  
                ...
                try {  
                    eventRepository.save(event);  
                    log.info("기존 데이터의 날짜와 현재 시간이 불일치합니다, 객체 생성 완료, Event ID: {}", event.getId());  
                } catch (Exception e) {  
                    log.error("기존 데이터의 날짜와 현재 시간 불일치합니다, - 객체 생성 실패", e);  
                    throw new CommonException("INIT-003", HttpStatus.INTERNAL_SERVER_ERROR);  
                }  
            } else {  
                log.info("현재 날짜의 데이터가 이미 존재합니다. 객체 생성 중지, 현재 데이터의 시간 : {}", storedEvent.getEventTime());  
            }  
        }  
    }  
}
